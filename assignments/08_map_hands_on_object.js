...
// TODO ES 6 FETURE
// by using map() method traverse only name of bank and add into the array
console.log(`\nby using map() method traverse only name of bank and add into the array`);
const array = [axisBank, sbiBank, iciciBank, kotakBank, hdfcBank, panjabBank];
const bankNameArray = array.map((element) => {
  return element.bankName;
});
console.log(`${bankNameArray}`);

// make list of all location 
const location = array.map((element)=>{
